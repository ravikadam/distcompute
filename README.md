...
* **WebAssembly SIMD Matmul Acceleration**: Incorporates a hand-written WebAssembly text (`matmul.wat`) kernel vectorized using `f32x4` SIMD instructions. It is compiled to a 361-byte binary embedded directly in `vm.js` as base64. Contiguous 2D and 3D batched matrix multiplications route through WebAssembly, resulting in a **~6.5× speedup** that removes the worker execution bottleneck. It falls back gracefully to JavaScript if WASM or SIMD is unsupported by the browser.
* **Mathematical Operations**: Custom implementations of broadcasting operations (`add`, `sub`, `mul`, `div`), reduction operations (`sum`, `mean` along specified axes), activations (`relu`, `gelu`), softmax, and categorical cross-entropy.
...
### 3. Run Training & Live Generation
...
4. **Live LLM Generation**: Open the **"Live LLM Generation"** card in the dashboard settings. Type a seed **Prompt** (e.g. `"To be"`), adjust the **Temperature** (`0` for greedy/deterministic decoding, or `0.7`–`1.0` for creative variation), and click **"Generate sample"**. The server will run a native JS forward pass and autoregressively stream the generated text.
5. To backup or export the trained model parameters, click the **"Download Weights"** button. This downloads a structured JSON file containing all weights, biases, and character mapping dictionaries.
